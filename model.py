...
class SRAMP(nn.Module):
    def __init__(self, mode='full', halfseqlen=400, oh=True):
        ...
        # token embedding
        self.token_emb = nn.Linear(6, utils.latent - utils.conv_len - utils.emb_len)

        # genome embedding MLP
        self.genometrans = nn.Linear(utils.emb_len, utils.emb_len)

        # Transformer
        self.trans = nn.Sequential(
            nn.TransformerEncoderLayer(
                utils.latent, 4, 4 * utils.latent,
        ...
        self.tail = nn.Sequential(
            nn.Dropout(0.3),
            nn.Linear(utils.latent, utils.latent // 4),
            Permute(0, 2, 1),
    ...
    def forward(self, x, emb, *args):
        ...
        if utils.half_length > self.halfseqlen:
            ...
            emb = emb[:, utils.half_length - self.halfseqlen:utils.halfseqlen + utils.halfseqlen + 1]
        ...
        # tail
        x = self.tail(x)
        x = torch.sigmoid(x[:, utils.half_length, :])
        return x
...
class SRAMP(nn.Module):
    def __init__(self, mode='full', halfseqlen=400, oh=True):
        ...
        self.trans = nn.Sequential(
            nn.TransformerEncoderLayer(utils.latent, 4, 4 * utils.latent, dropout=0.3, activation='gelu',
                                       batch_first=True, norm_first=True),
        )

        #
        self.gru = nn.GRU(utils.latent, utils.latent // 2, num_layers=2,
                          bidirectional=True, batch_first=True, dropout=0.3)

        #  LSTM
        self.lstm = nn.LSTM(utils.latent, utils.latent // 2, num_layers=2,
                            bidirectional=True, batch_first=True, dropout=0.3)
    ...
    def forward(self, x, emb, *args):
        ...
        x = self.tail(x)
        x = torch.sigmoid(x[:, utils.half_length, :])  # shape: [B,1]
    #    if x.dim() == 1:
      #      x = x.unsqueeze(1)  # 保证输出是二维 [B,1]
        return x
...
#
#         self.trans = nn.Sequential(
#             nn.TransformerEncoderLayer(utils.latent, 4, 4 * utils.latent, dropout=0.1, activation='gelu',
#                                        batch_first=True, norm_first=True),
#         )
#
#         #
